...
class PromotionController extends HController
{
	...
	public function actionIndex()
	{
		...
		$show_km=PPromotion::model()->getpromotion($vt,$limit);
		
		$this->render('index',array('show_km'=>$show_km,'lst'=>$lst));
	}
	public function actionDetailPromotion($title,$id,$lang)
	{	
		//share faceboook	
			$data_face = PPromotion::model()->findByPK($id);
			$this->pageOgImg 		= Yii::app()->params['url_base_http']."/upload/post/promotion/lg/".$data_face->image;
			if($lang=='vi'){
		...
		$id_promotion=$id;
		if(isset(Yii::app()->session['page_promotion']) &&  Yii::app()->session['page_promotion'] = $id)
		{
			
		}else{
			Yii::app()->session['page_promotion'] = $id;
			
			$view = PPromotion::model()->findByPK($id_promotion);

			$total= $view['total_view'];
			$total++;
			PPromotion::model()->updateByPK($id,array('total_view'=>$total));
		}
		
		$promotion = PPromotion::model()->promotion_details($id);
		$list_total_view = PPromotion::model()->promotion_total_view();
		$this->render('detailpromotion',array('promotion'=>$promotion,'list_total_view'=>$list_total_view));
	}
	...
	public function actionSearch()
	{
		if(isset($_POST['keyword']) && isset($_POST['curpage']))
		{
			...
			$news_line = PPromotion::model()->promotion_list_pagination($curpage,$keyword);

			//title cua danh sach
			$search = "Kết quả tìm kiếm";
			//danh sach tags hien thi
			$news_new = PPromotion::model()->findAllByAttributes(array('status'=>1));
			$this->renderPartial('list_promotion',array('new'=>$news_new,'news_line'=>$news_line,'search'=>$search,'lst'=>$lst));
		}
	}
	public function actionSearchEN()
	{
		if(isset($_POST['keyword']) && isset($_POST['curpage']))
		{
			$p = new PPromotion;
			...
			$news_line = PPromotion::model()->promotion_list_pagination_en($curpage,$keyword);
			$search = "Search results";
			$news_new = PPromotion::model()->findAllByAttributes(array('status'=>1));
			$this->renderPartial('list_promotion_en',array('new'=>$news_new,'news_line'=>$news_line,'lst'=>$lst,'search'=>$search));
		}
	}
}
?>