...
foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {  
    ...
    for ($row=2; $row<=$highestRow; $row++) {  
        ...
        //$bien .= $code."|".$service.'||';
        $service_type_arr =CsServiceType::model()->findAll("code = '".trim($code)."'");
		if(count($service_type_arr) <= 0){
			$sql = "INSERT INTO cs_service_type(code, name) VALUES ('".$code."', '".$service."')";  
			mysqli_query($connect, $sql);  
        }else{
			$bien .= '<tr><td>'.$code."</td><td>".$service."</td></tr>";
		    $uploadedStatus = 0;
		}	
    }  
} 
...
Import Excel file into Sevice Type</td></tr>
