<script>
	...
	var Item = <?php echo CJSON::encode($this->renderPartial('item_detail', array('quote_services' => $quote_services, 'teeth' => $teeth, 'form' => $form, 'i' => 'iTemp'), true)); ?>;
	var teethRemain = '<?php echo $teeth; ?>';

	var table = $('#sProduct table');
	var wrapper = $("#sProduct tbody");
...
/* ------------  TONG GIA TRI DICH VU  ------------ */
	function getSumAmount() {
		...
		$('#frm-quote .group_amount').each(function() {
		...
		$('#frm-quote #Quotation_sum_amount').autoNumeric('set', sum);
		$('#frm-quote #Quotation_sum_amount_usd').autoNumeric('set', sumUSD);
	}
...
/* ------------  KHOI TAO GIA TRI --------------- */
	...
	var segment =  $('.choseSeg').val();
	var segmentText = $('.choseSeg option:selected').text();

	$('.create_date').val(today);
	$('.complete_date').val(today);
	$('.segTxt').val(segmentText);
...
/* ------------  XU LY LUU TRU ---------------- */
	$('form#frm-quote').submit(function(e) {
		...
		$('form#frm-quote input, form#frm-quote select').each(function() {
