...
class Metadata extends CApplicationComponent {
    ...
    */
    ...
    /**
     * Get list modules of controllers with actions
     *
     * @param mixed $module
     * @return array
     */
    function getModulesControllersActions()
    {
        $data = array();
        $i    = 0;
        foreach($this->getModules() as $item => $module){
            foreach ($this->getControllersActions($module) as $k => $controller)
            {
                $data[$i] = $controller;
                $i++;
            }
