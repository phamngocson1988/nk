...
class Sms extends CActiveRecord {
    ...
    public $sendSMSError = array(
        '-2' => 'Sai so dien thoai',
        '-3' => 'Loi database',
        '-4' =>  'Username/ password khong dung',
        '-5' =>  'Sai loai tin',
        '-6' => 'Sai brandname'
    );
    #endregion
    ...
    //             );
    //             break;
    //         case 'ngt.lichhen2000.com':
    //             return array(
    //                 'url'       => 'http://14.241.253.96:8083/brandnamews.asmx?wsdl',
    //                 'username'  => 'nhakhoa2000ws2',
    //                 'password'  => 'Nh@Kh0@Cs2%#!',
    //                 'brandname' => 'NhaKhoa2000',
    //                 'loaitin'   => 1
    //             );
    ...
    function sendSms($phone, $text, $id_branch, $id_author, $author, $id_customer, $customer, $id_schedule, $type = 1, $source = 1) {
        // $SmsConnectInfo = $this->getSmsConnectInfo();
        ...
        $smsResult = Yii::app()->sms->send($phone, $text, 'SendMt');
        $idSms     = $smsResult->SendMtResult;
        $id_sms = 0;
        $status = 1;
        $smsRs = ($smsResult->SendMtResult) * -1;
    ...
    #region --- GUI TIN NHAN TEST
    function sendSmsTest($phone, $text, $id_branch, $id_author, $author, $id_customer, $customer, $id_schedule, $type = 1, $source = 1) {
        $idSms = rand(1000, 9999999999);
    ...
    #region --- LUU TIN NHAN
    function saveSMS($smss) {
        $sms = new Sms();
        $sms->attributes = $smss;

        unset($sms->create_date);
        if ($sms->validate() && $sms->save()) {
            return array('status' => 1, 'sms' => $sms->attributes);
        }
        return array('status' => -1, 'sms' => $sms->getErrors());
    }

    function saveSMSTest($smss) {
        $sms = new SmsTest();
        $sms->attributes = $smss;
