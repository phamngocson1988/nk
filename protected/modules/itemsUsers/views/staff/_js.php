...
$('#frm-add-customer').submit(function(e) {
    ...
    if (!formData.checkValidity || formData.checkValidity()) {
        jQuery.ajax({        	
            type:"POST",
            url: baseUrl+'/itemsUsers/Staff/addNewStaff',	
            data:formData,
            datatype:'json',
            success:function(data){ 
                if(data == -5){                
                    $('#parsley').html('Tài khoản đã tồn tại.');                
                    e.stopPropagation();               
                } else if(data > 0){ 
                    $('#addnewCustomerPopup').hide();  
...
<script type="text/javascript">
    ...
    function change_time_start(j,dow,id,id_dentist)
    {
        ...
        // return false;
		/*
        if(end_ca >= start_ca || $id_j == 0 )
        {	
        
    	    var d = new Date('01/01/2017 '+$value);
		    var start_1ca = d.getTime();
		    
		    var d = new Date('01/01/2017 '+$time_end);
		    var end_1ca = d.getTime();

            if(end_1ca >= start_1ca){*/
    ...
    function change_time_end(j,dow,id,id_dentist)
    {
        ...
        console.log(end_ca);
        /*
        if(start_next <= start_ca )   //so sanh thời gian start  vs end ca trong 1 ca làm việc
        {
