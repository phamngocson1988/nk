...
class PromotionController extends Controller
{
	...
	public function init() {
        $this->link = Yii::app()->params['url_base_http']."/itemsPost/promotion/update/id/"; 
    }
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['Promotion']))
		{
			...
			$hinh=$_FILES["Promotion_images"]["error"]==0?$_FILES["Promotion_images"]["name"]:"";
			if($hinh != ""){
			$duoi = explode('.',$hinh);
			...
			if($filext!=""){
				...
				$kq=move_uploaded_file($_FILES["Promotion_images"]["tmp_name"],"upload/post/promotion/lg/$ten_hinh");
				
				$img = new SimpleImage("upload/post/promotion/lg/$ten_hinh");
				$width=ceil($img->getWidth()/2);
				$height=ceil($img->getHeight()/2);
				$img->resize($width,$height);
				$img->save("upload/post/promotion/md/$ten_hinh");	
				
				$img = new SimpleImage("upload/post/promotion/md/$ten_hinh");
				$width=ceil($img->getWidth()/2);
				$height=ceil($img->getHeight()/2);
				$img->resize($width,$height);
				$img->save("upload/post/promotion/sm/$ten_hinh");
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Promotion']))
		{
		    ...
		    if($filext!=""){
				...
				if($xoa_hinh!=""){
				   unlink("upload/post/promotion/lg/$xoa_hinh");
				   unlink("upload/post/promotion/md/$xoa_hinh");
				   unlink("upload/post/promotion/sm/$xoa_hinh");
				}
			}
			if($model->save()){
                echo '<script type="text/javascript">'; 
	...
	*/
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
		    ...
		    //$idm=substr($_SERVER['REQUEST_URI'],-1);
		   
		    $image =Promotion::model()->findAll('id=:st',array(':st'=>$id));
		    if(count($image)!=0 and $image[0]['image']!=""){
			   $xoa=$image[0]['image'];
			   unlink(Yii::app()->basePath.'/../upload/post/promotion/lg/'.$xoa);
			   unlink(Yii::app()->basePath.'/../upload/post/promotion/md/'.$xoa);
			   unlink(Yii::app()->basePath.'/../upload/post/promotion/sm/'.$xoa);
			   //unlink("upload/post/promotion/$xoa");
		   }
			//echo $image[0]['image'];exit; 
