...
<style type="text/css">
	.thumbail{width: 100% !important;}
	.img-main{width: 100% !important;}
	#thumbail div{ 	padding-right: 0px;}
	.defaulf-padding-left{padding-left: 0px;}
	.name_product{font-weight: bold;font-size: 30px;}
	.title{font-size: 17px;font-weight: bold;}
	.content_title{color: #919190;}
	.product_item img{height: 300px;margin-left: 25px;}
	.btn{font-size: 13px;}
...
<div class="container">
	<div style="margin-top: 40px" class="row">
	    <div class="col-sm-6" style="color: #19a8e0; font-size: 20px;font-weight: bold;">
	   		<?php echo Yii::t('translate','all_product'); ?>
	    </div>
	    <div class="search col-sm-6" style="text-align: right; font-size: 12px;color: #ccc">
	        <ul>
	            <li>
	                <div>
	                	<?php echo Yii::t('translate','cart'); ?> : 0 <?php echo Yii::t('translate','product'); ?>
	                </div>
	            </li>
	        </ul>
	    </div>
		...
		<div class="col-xs-12 col-sm-5">
			...
			<div class="col-xs-12 col-sm-12 defaulf-padding-left " style="">
				<div class="col-xs-5 col-sm-6 defaulf-padding-left">
					<button style="width: 100%;margin:15px 0px;" type="button" class="btn btn_green" onclick="addShopingCart(<?php echo $list['id'].",  '".$list['name']."', '".$list['price']."' , '".$list['stock']."'" ?>);"><?php echo Yii::t('translate','buy_now'); ?></button>
				</div>
				<div class="col-xs-7 col-sm-6 defaulf-padding-left">
					<button style="margin:15px 0px; width: 100%;" type="button" class="btn btn_green" onclick="addExpressShopingCart(<?php echo $list['id'].",  '".$list['name']."', '".$list['price']."' , '".$list['stock']."'" ?>);" ><?php echo Yii::t('translate','add_cart'); ?></button>
				</div>
			</div>
		</div>
		<div class="col-xs-12 col-sm-7 row">
			...
			<div class="row" style="margin-top: 30px">
				<div class="col-xs-6 col-sm-6">
					<p class="title"><?php echo Yii::t('translate','price'); ?></p>
					<p class="content_title" style="color: red; font-size: 20px; "><?php echo number_format($list['price'],0, ',', '.');?> VNĐ</p>
				</div>
				
				<div class="col-xs-6 col-sm-6">
					 <div class="weight">
						<p class="title"><?php echo Yii::t('translate','weight'); ?></p>
						<p class="content_title"><?php echo $list['unit'] ?></p>
					</div>
				</div>
				<div class="col-xs-12 ">
					<p class="title"><?php echo Yii::t('translate','uses'); ?></p>
					<div class="content_title ">
						<?php if($lang=='vi'){echo $list['description'];}else{echo $list['description_en'];}?>
					</div>
				</div>
				<div class="col-xs-12 " >
					<p class="title"><?php echo Yii::t('translate','information_detail'); ?></p>
					<div class="content_title ">
						<?php if($lang=='vi'){echo $list['instruction'];}else{echo $list['instruction_en'];}?>
					</div>
				</div>
	...
	<div class="row" style="padding-left: 15px;margin-top: 60px;margin-bottom: 30px">
		<p style="font-size: 25px;font-weight: bold;"><?php echo Yii::t('translate','product_type'); ?></p>
		<?php
		...
		<?php  
			foreach ($sort as $key => $v) {
		 	$title = $this->convert_vi_to_en($v['name']);
    		$id = $v['id']; 
    	?>
			<div class="item ">
				<div class="product_item">
					<a href="<?php echo Yii::app()->request->baseUrl; ?>/san-pham/chi-tiet/<?php echo $title;?>-<?php echo $id;?>/lang/<?php echo $lang;?>">
						<img  class="img-responsive"  src="<?php echo Yii::app()->request->baseUrl; ?>/upload/<?php echo $v['url_action'] ?>/lg/<?php echo $v['name_upload'] ?>" > 
						<div align="center" class="product_title">
			                <p class="name"><?php if($lang=='vi'){echo $v['name'];}else{echo $v['name_en'];} ?></p>
...
<script>
	function addShopingCart(idProduct,name, price, stock){
    	...
    	jQuery.ajax({
                type : "POST",
                url : "<?php echo CController::createUrl("shopping/addShoppingCart");?>",
                data : {
                    "idProduct" : idProduct,
                    "name"      : name,
                    "price"     : price,
