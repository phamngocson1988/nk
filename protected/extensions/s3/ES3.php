 ...
 class ES3 extends CApplicationComponent
{
	...
	public function deleteObject($uri, $bucket="") {
		if( $bucket == "" )
		{
			$bucket = $this->bucket;
		}
		...
		{
			throw new CException('URI param cannot be empty');
		}
	...
	public function getObject($uri, $bucket = "") {
		if( $bucket == "" )
		{
			$bucket = $this->bucket;
		}
	...
	public function getObjectInfo($uri, $bucket = "") {
		if( $bucket == "" )
		{
			$bucket = $this->bucket;
		}
		...
		$s3 = $this->getInstance();
		return $s3->getObjectInfo($bucket, $uri);
	}
