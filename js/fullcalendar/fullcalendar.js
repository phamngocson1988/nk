...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
		...
		while (slotTime < this.maxTime) {
			...
			html +=
				'<tr data-time="' + slotDate.format('HH:mm:ss') + '"' +
					...
					(isRTL ? axisHtml : '') +
						this.renderTimeRowHtml(isLabeled,slotDate) +
				"</tr>";
	...
	renderTimeRowHtml: function(isLabeled,slotDate) {
		var timeTd = '';
		var ws = $(window).width();
		var wdv = ws/this.colCnt;
		for (i = 0; i < this.colCnt; i++) {
			timeTd += 
				(isLabeled ?
				'<div class="text-time" style="width='+wdv+'px;">' + // for matchCellWidths
					htmlEscape(slotDate.format(this.labelFormat)) +
				'</div>' :
				''
				);
...
function Calendar_constructor(element, overrides) {
	...
	function initialRender() {
		...
		content = $("<div class='fc-container'/>").prependTo(element);

		content = $("<div class='fc-view-container'/>").prependTo(content);
